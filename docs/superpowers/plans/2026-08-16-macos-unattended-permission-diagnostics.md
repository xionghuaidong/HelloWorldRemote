...
Keep the current Python subprocess implementation, but add a safe status-path argument. Before each Python exit, atomically replace the status file with one of:
