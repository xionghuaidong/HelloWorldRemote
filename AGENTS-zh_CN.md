...
本项目使用适用于 Codex 的 Superpowers 插件。

## 启动检查

在每个新会话开始时：

1. 检查 `using-superpowers` skill 是否可用且可以调用。
2. 如果 `using-superpowers` 不可用或无法调用，不要假装 Superpowers 已启用。
3. 告知用户：

   > 本项目要求使用 Superpowers，但它尚未安装或启用。
   > 打开 `/plugins`，搜索 `superpowers`，安装并启用它，然后启动一个新的 Codex 会话。
...
* 在设计重要的新行为之前使用 `brainstorming`，让它通过提问把一个粗略的想法整理成规格说明，并在它分段呈现设计时逐段确认，而不是跳过设计直接写代码。
* 设计获得批准后，使用 `using-git-worktrees` 在一个新分支上建立一个独立的工作区，避免直接在起始分支上进行实现。
* 在进行多步骤实现之前使用 `writing-plans`。计划应被拆分为一个个小任务，每个任务都有明确的文件路径、代码和验证步骤。
* 使用 `subagent-driven-development` 来执行已批准的计划，为每个任务派发一个全新的 subagent 并在每一步进行审查；如果更倾向于按批次执行并设置人工检查点，则改用 `executing-plans`。
* 对行为变更和 bug 修复遵循 test-driven development（`test-driven-development`）：先写一个失败的测试，确认它失败，再写出让它通过的最小代码，然后重构。
* 对测试失败和意外行为使用 `systematic-debugging`，而不是临时性的修补。
* 在任务之间以及合并之前使用 `requesting-code-review`。立即修复 Critical 问题，并在继续执行之前解决 Important 问题；Minor 问题可以推迟处理。
* 在处理 code review 反馈之前使用 `receiving-code-review`，尤其是在反馈不明确或技术上存疑时。
* 在声称工作已完成、问题已修复或检查已通过之前使用 `verification-before-completion`。
* 在工作完成并经过最新验证后使用 `finishing-a-development-branch`。
