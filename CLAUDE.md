...
`.claude/settings.json` requests that Superpowers be enabled for this project. This setting does not by itself prove that the plugin is installed or loaded successfully. When this folder is trusted, Claude Code should prompt to install the plugin if needed. At the beginning of a new session:

1. Check whether the `superpowers:using-superpowers` skill is available and can be invoked.
2. If `superpowers:using-superpowers` is unavailable or cannot be invoked, do not pretend that Superpowers is active.
3. Tell the user:

   > Superpowers is required for this project but is not installed or enabled.
   > Run `/plugin install superpowers@claude-plugins-official`, then run `/reload-plugins` or start a new Claude Code session.
...
* Use `superpowers:brainstorming` before designing significant new behavior. Let it turn a rough idea into a spec through questions, and review the design in the sections it presents rather than skipping ahead to code.
* Once a design is approved, use `superpowers:using-git-worktrees` to set up an isolated workspace on a new branch, so implementation doesn't happen directly on the branch you started from.
* Use `superpowers:writing-plans` before multi-step implementation. Expect the plan to be broken into small tasks with exact file paths, code, and verification steps.
* Use `superpowers:subagent-driven-development` to carry out an approved plan, dispatching a fresh subagent per task with review at each step; use `superpowers:executing-plans` instead when batch execution with human checkpoints is preferred.
* Follow test-driven development (`superpowers:test-driven-development`) for behavior changes and bug fixes: write a failing test, watch it fail, write the minimal code to pass it, then refactor.
* Use `superpowers:systematic-debugging` for test failures and unexpected behavior instead of ad hoc fixes.
* Use `superpowers:requesting-code-review` between tasks and before merging. Fix Critical issues immediately and resolve Important issues before proceeding; Minor issues may be deferred.
* Use `superpowers:receiving-code-review` before acting on review feedback, especially when the feedback is unclear or technically questionable.
* Use `superpowers:verification-before-completion` before claiming that work is complete, fixed, or passing.
* Use `superpowers:finishing-a-development-branch` once work is complete and freshly verified.
...
* Also verify that `.claude/settings.json` parses as JSON and enables only the expected Superpowers plugin; automated checks cover only machine-readable structure; review confirms complete shared instruction policies, accurate README workflow facts, and faithful capture prompts; historical English documents differ from their base versions only by language navigation, except that this authorized migration replaces the known custom code with exactly six lowercase x characters, `xxxxxx`; currently runnable repository tests pass; `git diff --check e30a65b..HEAD` passes; and the final diff contains only approved environment and documentation changes.
